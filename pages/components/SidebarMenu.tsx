...
const SidebarMenu: React.FC = () => {
  ...
  useEffect(() => {
    ...
    const observer = new IntersectionObserver(
      (entries) => {
        entries.forEach((entry) => {
          if (entry.isIntersecting) {
            setActiveSection(entry.target.id); // Set the active section using the element's ID
          }
        });
      },
  ...
  return (
    <div className="hidden lg:flex flex-col p-4 items-start text-black">
      {menuItems.map((item) => (
        <Link key={item.name} href={item.href}>
          <span
            className={`group flex items-center mb-4 text-md font-semibold transition-all duration-300 hover:text-black cursor-pointer ${
          ...
          >
            ...
            <span
              className={`block h-px transition-all duration-300 ${
                activeSection === item.href.slice(1)
                  ? "w-12 bg-black" // Extend line if section is active
                  : "w-4 bg-gray-500 group-hover:w-12 bg-black" // Shorter line when not active but extend on hover
              }`}
            ></span>
            {/* Menu Item Text */}
            <span className="ml-2">{item.name}</span>
